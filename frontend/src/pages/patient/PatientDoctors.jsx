      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
        {['All Specialties', 'Cardiology', 'General Physician'].map(specialty => (
          <button
            ...
            className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-medium whitespace-nowrap transition-colors text-sm md:text-base ${
        ...
        <button className="px-4 md:px-6 py-2 md:py-3 rounded-full font-medium whitespace-nowrap bg-white text-gray-700 border border-gray-200 flex items-center gap-2 text-sm md:text-base">
        ...
        {filteredDoctors.map(doctor => (
          <div key={doctor.id} className="bg-white rounded-2xl p-4 md:p-5 shadow-sm border border-gray-100">
            <div className="flex items-start gap-4 mb-4">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-teal-100 to-cyan-100 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shrink-0">
                {doctor.avatar}
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-base md:text-lg text-gray-800 mb-1">{doctor.name}</h3>
      ...
      {selectedDoctor && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end md:items-center justify-center z-50 p-0 md:p-4">
          <div className="bg-white w-full md:max-w-2xl md:rounded-3xl rounded-t-3xl max-h-[90vh] overflow-y-auto">
            {/* Header */}
            <div className="bg-gradient-to-br from-teal-400 to-cyan-500 text-white p-6 md:rounded-t-3xl rounded-t-3xl">
              <button 
                onClick={() => setSelectedDoctor(null)}
                className="mb-4 p-2 hover:bg-white hover:bg-opacity-20 rounded-xl transition-colors inline-flex"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                </svg>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 mb-6 pb-6 border-b">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-teal-100 to-cyan-100 rounded-2xl flex items-center justify-center text-3xl md:text-4xl">
                  {selectedDoctor.avatar}
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-lg md:text-xl text-gray-800">{selectedDoctor.name}</h3>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-gray-800 text-base md:text-lg">Select Date</h3>
                </div>
                <div className="flex gap-2 md:gap-3 overflow-x-auto pb-2">
                  {dates.map((date, idx) => (
                    <button
                      ...
                      className={`shrink-0 w-16 md:w-20 py-3 md:py-4 rounded-xl border-2 transition-all ${
                    ...
                    >
                      ...
                      <div className="text-lg md:text-xl font-bold text-gray-800">{date.date}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-gray-800 text-base md:text-lg">Select Time</h3>
                </div>
                <div className="grid grid-cols-3 gap-2 md:gap-3">
                  {timeSlots.map((time, idx) => (
                    <button
                      ...
                      className={`py-2 md:py-3 rounded-xl border-2 font-medium transition-all text-sm md:text-base ${
                        selectedTime === time
              ...
              {/* Consultation Fee */}
              <div className="bg-teal-50 rounded-2xl p-4 md:p-5 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-teal-700">
                    <span className="text-2xl md:text-3xl">💵</span>
                    <span className="font-medium text-sm md:text-base">Consultation Fee</span>
                  </div>
                  <span className="text-2xl md:text-3xl font-bold text-teal-600">${selectedDoctor.fee}</span>
                </div>
              </div>
